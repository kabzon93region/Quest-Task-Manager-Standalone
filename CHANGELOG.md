...
### Добавлено
- Скрипт автоматической публикации `scripts/publish-github.cmd` — коммит, push, сборка APK, создание GitHub Release

## [2.5.4] — 2026-08-23

### Исправлено
- **IPv6 loopback** — подключение через `::1` (IPv6) вместо только `127.0.0.1` (IPv4). На Quest adbd слушает на `::` (IPv6 all interfaces), IPv4 loopback мог не работать
- Улучшен статус-бар: вместо «Wireless debugging выключен» показывает «подключение не удалось — проверьте debug-порт»
- Убран ненадёжный mDNS (возвращает 5555) из flow после pairing
- Детальное логирование попыток подключения

## [2.5.3] — 2026-08-23

### Исправлено
- **Debug-порт после pairing** — после успешного сопряжения приложение пробует подключиться к pairing-порту, 5555 и соседним портам для поиска adbd. Если probe нашёл порт — сразу подключается. Если нет — пробует mDNS, затем просит ввести вручную
- mDNS на Quest возвращает 5555 (неверный порт) — probe добавлен как основной метод
- Исправлен парсинг UID в `/proc/net/tcp6` (позиция [9] вместо [7])

## [2.5.2] — 2026-08-23

### Исправлено
- Debug-порт после pairing: mDNS + /proc/net/tcp auto-discovery (заменён на probe в 2.5.3)
